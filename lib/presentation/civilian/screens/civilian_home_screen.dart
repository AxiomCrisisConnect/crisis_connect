...
class _CivilianHomeScreenState extends ConsumerState<CivilianHomeScreen>
  ...
  Future<void> _triggerSOS() async {
    ...
    try {
      ...
      final lat = pos?.latitude ?? 12.9716;  // fallback to Bangalore
      ...
      // Auto-assign nearest Rescue + Medical volunteers (mock)
      await _assignVolunteers(request, repo);
  ...
  Future<void> _assignVolunteers(
      EmergencyRequest request, EmergencyRepository repo) async {
    // TODO: In production, query Firestore for available volunteers and assign via Cloud Function
    // MOCK: Create a mock assignment
    final assignment = Assignment(
      id: const Uuid().v4(),
      emergencyRequestId: request.id,
      volunteerId: 'mock_volunteer_1',
      volunteerName: 'Dr. Arun Kumar',
      volunteerSkills: ['Medical:Doctor', 'Rescue'],
      civilianId: request.civilianId,
      civilianName: request.civilianName,
      emergencyLatitude: request.latitude,
      emergencyLongitude: request.longitude,
      assignedAt: DateTime.now(),
      status: AssignmentStatus.pending,
    );
    await repo.createAssignment(assignment);

    // TODO: Send FCM push notification to volunteer
    // await ref.read(notificationServiceProvider).sendAssignmentNotification(...)
  }
  ...
  Widget _buildHomeTab(BuildContext context, String name) {
    return ListView(
      ...
      children: [
        ...
        // SOS Button — massive, glowing, pulsing
        Center(
  ...
  Widget _buildProfileTabCivilian(BuildContext context, String name) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        // TODO: Add Medical Form (pre-fill blood group, allergies, conditions)
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(isSOS ? 'SOS Alert' : (request.helpCategory?.label ?? 'Help Request'),
